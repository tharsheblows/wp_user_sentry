...
class Admin {
	...
	static function adminInit() {
		...
		add_settings_section(
			...
			__( 'General Settings' ),
			array( __NAMESPACE__ . '\Admin', 'settingsCallback' ),
			'wp-user-sentry-settings'
		);
		add_settings_field(
			'geo_api_service',
			__( 'Look Up Country by IP?' ),
			array( __NAMESPACE__ . '\Admin', 'fieldApiRender' ),
			'wp-user-sentry-settings',
			'wp_user_sentry_settings_section'
		);
		add_settings_field(
			...
			array( __NAMESPACE__ . '\Admin', 'fieldRolesRender' ),
			'wp-user-sentry-settings',
			'wp_user_sentry_settings_section'
		);
		add_settings_field(
			...
			array( __NAMESPACE__ . '\Admin', 'fieldRepeatRender' ),
			'wp-user-sentry-settings',
			'wp_user_sentry_settings_section'
		);
		add_settings_field(
			...
			array( __NAMESPACE__ . '\Admin', 'fieldEmailSubjectRender' ),
			'wp-user-sentry-settings',
			'wp_user_sentry_settings_section'
		);
		add_settings_field(
			...
			array( __NAMESPACE__ . '\Admin', 'fieldEmailRender' ),
			'wp-user-sentry-settings',
			'wp_user_sentry_settings_section'
		);
		add_settings_field(
		...
		* Test Email Hooks
		*/
		add_action( 'wp_ajax_wp-user-sentry-test-email', array( __NAMESPACE__ . '\Admin', 'sendTestEmailCallback' ) );

		add_action( 'admin_footer', array( __NAMESPACE__ . '\Admin', 'jsFooterRender' ) );
	 ...
	 * @since 0.4.0
	 */
	static function settingsCallback() { // @phpcs:ignore
		?>
	<p><?php _e( 'WP User Sentry provides additional login monitoring, notifying users of a successful login and providing a sessions table for each user in their profile.', 'wp-user-sentry' ); // @phpcs:ignore ?></p>
		<?php
	}
	 ...
	 * @since 0.4.0
	 */
	static function fieldApiRender() { // @phpcs:ignore
	 ...
	 * @since 0.4.0
	 */
	static function fieldRolesRender() { // @phpcs:ignore
		$options = get_option( 'wp_user_sentry_settings' );
	 ...
	 * @since 0.4.0
	 */
	static function fieldEmailRender() { // @phpcs:ignore
		$options = get_option( 'wp_user_sentry_settings' );
...
';
	 ...
	 * @since 1.1.0
	 */
	static function fieldEmailSubjectRender() { // @phpcs:ignore
		$options = get_option( 'wp_user_sentry_settings' );
	 ...
	 * @since 1.1.0
	 */
	static function jsFooterRender() { // @phpcs:ignore
		$screen = get_current_screen();
		if ( $screen->id !== 'settings_page_wp-user-sentry-settings' ) {
			return;
		}
	 ...
	 * @since 1.1.0
	 */
	static function sendTestEmailCallback() { // @phpcs:ignore
	...
	/**
	 * Test to see if we can use the geolocation api.
	 *
	 * @return bool True if it's possible to use, false otherwise.
	 */
	public static function testMaxMindPresence() { // @phpcs:ignore
		$return             = false;
		$wooCommerce_GeoAPI = get_option( 'woocommerce_maxmind_geolocation_settings' ); // @phpcs:ignore
		if ( class_exists( 'woocommerce' ) && ! empty( $wooCommerce_GeoAPI ) && isset( $wooCommerce_GeoAPI['license_key'] ) ) {
			$return = true;
		}
		return $return;
	}
